...
setwd("C:\Userpwds\aron\Google Drive\BA\thesis\data_analysis\r_lme_analysis")
#load data
data <- read.csv("corr_results.csv", header=T, comment.char="#", stringsAsFactors=F)
...
data <- filter(data, !data$participant%in%c(1:56,64,79,90,94,123))
...
task.fillers$choice_response.corr[task.fillers$participant%%2==0 & !task.fillers$participant<56]<-ifelse(task.fillers$choice_response.corr[task.fillers$participant%%2==0 & !task.fillers$participant<56]=="incorrect", "correct", "incorrect")

table(task.fillers$participant,task.fillers$choice_response.corr)
...
boxcox(choice_response.rt~cond*position*participant, data=task.rt) #use sqrtRT
...
# summary(m1.rt)
#singular fit, simplify random effects structure:
...
## Analysis of distortion responses (add here)
