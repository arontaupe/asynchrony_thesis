---
title: "AV-Experiment Pilot - BA thesis Aron Petau"
#output: html_notebook
---

This is my analysis script.
It will apply a mixed linear effects model on the raw results of the audiovisual asynchrony study.
...
# Pilot exclusion
data <- filter(data, !data$participant %in% c(1:56)) 
# Performance exclusion
data <- filter(data, !data$participant %in% c( 79, 90)) 
#Framerate exclusion
data <- filter(data, !data$participant %in% c(167, 168, 64, 180, 99, 167)) 
#1-56 are pilot, the other ones had low accuracy on the task
...
```
Notably, the 400ms (with and without echo) conditions were not consistently recognized as asynchronous. I therefore decided to look at individual participants' responses, too:

```{r}
table(sj.clean$cond,sj.clean$participant,sj.clean$response_sync.corr)

```
Some participants consistently rated the 400ms conditions as synchronous.

***Plot of distortion judgment responses across all three blocks***
```{r}
...
***Plot of simultaneity judgment responses across all three blocks***
```{r}
...
###2. Main task accuracy: Descriptives
```{r}
...
coefs$p <- format.pval(2*(1 - pnorm(abs(coefs$t.value))), digits = 2, eps = 0.0001) #calculate p-value, round to 2 digits
coefs
...
**check for interaction with position**
```{r}
...
```
Here's (roughly) how to interpret the coefficients table of the model with interaction:

Fixed effects:
                                        Estimate Std. Error t value  
(Intercept)                             43.84904    0.67116  65.333  
cond10ms delay, no echo                 -0.16970    0.33736  -0.503  
cond400ms delay, no echo                 4.45227    0.34206  13.016  
cond10ms delay, echo                     0.23441    0.34187   0.686  
cond400ms delay, echo                    2.75384    0.33921   8.118  
positionsecond                          10.37216    0.33766  30.717  
trial_counter_choice.scal               -1.21023    0.07643 -15.834  
cond10ms delay, no echo:positionsecond   0.03364    0.47704   0.071  
cond400ms delay, no echo:positionsecond -1.33879    0.48104  -2.783  
cond10ms delay, echo:positionsecond     -0.49606    0.48066  -1.032    
cond400ms delay, echo:positionsecond     0.76639    0.47832   1.602  

Remember: The reference level is the "no delay, no echo" condition. This time ****if the entity asked for was the first in the sentence**** (note how I entered the position factor as "position", not "position.cont" in the model? This means we didn't use the sum-coded position factor, which allows us to see how "position" may affect each condition differently)

So, line 1, for instance:
cond10ms delay, no echo --> comparison between "no delay, no echo, first entity asked for" and "10ms delay, no echo, first entity asked for"

positionsecond --> effect of whether we're asking for the first or second entity in the "no delay, no echo" reference condition

Interactions, e.g. "cond10ms delay, no echo:positionsecond" --> Is the difference between comparison between "no delay, no echo, **first** entity asked for" and "10ms delay, no echo, **first** entity asked for" ***bigger/larger than*** the difference between comparison between "no delay, no echo, **second** entity asked for" and "10ms delay, no echo, **second** entity asked for"


For m2, given that we changed the contrast coding to sum-coded comparisons, the interpretation is thus:

Fixed effects:
                             Estimate Std. Error t value
(Intercept)                  45.64192    0.63111  72.320
contrast10ms                 -0.33035    0.36918  -0.895
positionsecond               10.12967    0.18158  55.785
contrast400ms                 1.71687    0.37081   4.630
trial_counter_choice.scal    -1.13435    0.09116 -12.444
contrast10ms:positionsecond   0.50094    0.51901   0.965
positionsecond:contrast400ms -2.02999    0.52137  -3.894

contrast10ms-->is there a difference between the two 10ms conditions (with, without echo) if the first entity is asked for?

positionsecond-->overall effect of position

contrast10ms:positionsecond-->Interaction; is the difference between the two 10ms conditions (with, without echo) if the ***first*** entity is asked for **bigger/smaller than** the difference between the two 10ms conditions (with, without echo) if the ***second*** entity is asked for? 
...
**Exploratory: check for interaction with position**
```{r}
...
## Analysis of distortion responses (plug in same type of code as for SJ analysis)
...
Export citation of packages for thesis
```{r}
citation()
citation("lme4")
citation("ggplot2")
